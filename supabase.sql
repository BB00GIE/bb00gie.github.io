...
drop policy if exists "Anyone can read resume profile" on public.resume_profile;
create policy "Anyone can read resume profile"
  on public.resume_profile for select
  using (true);

drop policy if exists "Allowed authors can manage resume profile" on public.resume_profile;
create policy "Allowed authors can manage resume profile"
  on public.resume_profile for all
  to authenticated
  using (exists (select 1 from public.allowed_authors where user_id = auth.uid() and github_username = 'BB00GIE'))
  with check (exists (select 1 from public.allowed_authors where user_id = auth.uid() and github_username = 'BB00GIE'));
